...
//매니저 수정 
	 ...
	 * 
	 * PreparedStatement insertCommonUserPstmt = null; PreparedStatement
	 * getUserIdxQueryPstmt = null; PreparedStatement getRoleIdxQueryPstmt = null;
	 * PreparedStatement insertUserRoleTablePstmt = null; PreparedStatement
	 * insertManagerPstmt = null; PreparedStatement getManagerNamePstmt = null;
	 * PreparedStatement insertAllowedIPPstmt = null;
	 * 
	 * String adminId = adminDTO.getAdminId();
	 * 
	 * AllowedIPDTO ipDTO = new AllowedIPDTO(); ipDTO = adminDTO.getIPList().get(0);
	 * String ipAddr = ipDTO.getIpAddress().toString();
	 * 
	 * int userIdx = -1; int roleIdx = -1;
	 * 
	 * String insertCommonUser = " INSERT INTO common_user (user_idx, user_type)
	 * VALUES (USER_IDX_SEQ.NEXTVAL, ?) "; String getUserIdxQuery = " SELECT
	 * USER_IDX_SEQ.CURRVAL FROM dual "; String getRoleIdxQuery = " SELECT role_idx
	 * FROM role WHERE role_name = 'ROLE_MANAGER' "; String insertUserRoleTable = "
	 * INSERT INTO user_role_table (user_idx, role_idx) VALUES (?, ?) "; String
	 * insertManager = " INSERT INTO admin ( user_idx, admin_id, admin_level,
	 * admin_pwd, admin_name, admin_email, manage_area, last_login_date, picture,
	 * is_active, tel) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )"; String
	 * insertAllowedIP = " INSERT INTO allowed_ip (allowed_ip_idx, admin_id,
	 * ip_address, created_at) " + "VALUES (ALLOWED_IP_SEQ.NEXTVAL, ?, ?, ?)";
	 * 
	 * try { con = dbCon.getDbConn(); con.setAutoCommit(false); //중간에 오류나면 다 빽해야
	 * 
	 * //1. common_user 데이터생성 try(PreparedStatement ps =
	 * con.prepareStatement(insertCommonUser)){ ps.setString(1, "ADMIN");
	 * ps.executeUpdate(); }
	 * 
	 * //2.user_idx 가져옴 try(PreparedStatement ps =
	 * con.prepareStatement(getUserIdxQuery); ResultSet rs = ps.executeQuery()){
	 * if(rs.next()) { userIdx = rs.getInt(1); } }
	 * 
	 * //3.role_idx 가져오기 try (PreparedStatement ps =
	 * con.prepareStatement(getRoleIdxQuery); ResultSet rs = ps.executeQuery()) { if
	 * (rs.next()) { roleIdx = rs.getInt(1); } }
	 * 
	 * //4.user_role_table 입력 try(PreparedStatement ps =
	 * con.prepareStatement(insertUserRoleTable)) { ps.setInt(1, userIdx);
	 * ps.setInt(2, roleIdx); ps.executeUpdate(); }
	 * 
	 * //5.관리자(admin) 입력 try (PreparedStatement ps =
	 * con.prepareStatement(insertManager)) { ps.setInt(1, userIdx); ps.setString(2,
	 * adminDTO.getAdminId()); ps.setString(3, adminDTO.getAdminLevel());
	 * ps.setString(4, adminDTO.getAdminPwd()); ps.setString(5,
	 * adminDTO.getAdminName()); ps.setString(6, adminDTO.getAdminEmail());
	 * ps.setString(7, adminDTO.getManageArea()); ps.setTimestamp(8,
	 * Timestamp.valueOf(adminDTO.getLastLoginDate())); ps.setString(9,
	 * adminDTO.getPicture()); ps.setString(10, adminDTO.getIsActive());
	 * ps.setString(11, adminDTO.getTel()); ps.executeUpdate(); }
	 * 
	 * // 6. allowed_ip 입력 try (PreparedStatement ps =
	 * con.prepareStatement(insertAllowedIP)) { ps.setString(1, adminId);
	 * ps.setString(2, ipAddr); ps.setTimestamp(3,
	 * Timestamp.valueOf(LocalDateTime.now())); ps.executeUpdate(); }
	 * 
	 * con.commit(); result = true;
	 * 
	 * } catch (SQLException e) { if(con != null) { try { con.rollback(); } catch
	 * (SQLException rollbackEx) { rollbackEx.printStackTrace(); } } throw e;
	 * 
