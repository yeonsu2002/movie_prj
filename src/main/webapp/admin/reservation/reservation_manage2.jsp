...
$(function(){
	$(".cancelReservation").click(function(){
		...
		var $statusCell = $(this); // 클릭한 상태 셀
		var $dateCell = $clickedRow.find("td:nth-child(5)"); // 예매/취소 날짜 셀
			...
			$.ajax({
				...
				success: function(response){
					if(response.trim() == "success"){
						...
						// 실시간으로 UI 업데이트
						$statusCell.removeClass("cancelReservation")
								   .addClass("canceled")
								   .text("❌ 취소 완료")
								   .css("cursor", "default");
						
						// 날짜 셀도 취소 스타일로 변경
						$dateCell.addClass("canceled");
						
						// 현재 날짜/시간으로 취소 날짜 업데이트
						var now = new Date();
						var cancelDate = now.getFullYear() + "-" + 
										String(now.getMonth() + 1).padStart(2, '0') + "-" + 
										String(now.getDate()).padStart(2, '0') + " " +
										String(now.getHours()).padStart(2, '0') + ":" + 
										String(now.getMinutes()).padStart(2, '0');
						$dateCell.text(cancelDate);
						
						// 클릭 이벤트 제거 (중복 취소 방지)
						$statusCell.off("click");
